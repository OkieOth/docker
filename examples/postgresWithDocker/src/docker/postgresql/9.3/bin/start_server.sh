...
containerName=psql_9_3_postresWithDocker
imageName=postgres
imageTag=9.3
...
if docker ps -a | grep "$containerName" > /dev/null; then
    ...
    docker run -p 127.0.0.1:6001:5432 --name "$containerName" -e POSTGRES_PASSWORD="$dbUserPwd" -e POSTGRES_USER="$dbUser" -e POSTGRES_DB="$dbName" -e PGDATA=/opt/pgdata -v ${dataDir}:/opt/pgdata -d "$imageName":"$imageTag" 
fi
